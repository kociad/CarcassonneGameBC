...
class GameSession:
    ...
    def generateCardsDeck(self) -> list:
        ...
        selectedCardSets = settingsManager.get("SELECTED_CARD_SETS", ["baseGame"])
        ...
        for setName in selectedCardSets:
            ...
            if setData['definitions']:
                ...
                logger.info(f"Loaded selected card set: {setName} with {len(setData['definitions'])} card definitions")
        ...
        logger.debug(f"Deck generated with {len(cards)} cards from {len(cardDefinitions)} card definitions")
        return cards
