...
class GameCommand:
    ...
    def __init__(self, command_type: str, player_index: int, timestamp: Optional[float] = None):
        self.command_id = str(uuid.uuid4())
        self.command_type = command_type
        self.player_index = player_index
        self.timestamp = timestamp or time.time()
        self.sequence_number = 0  # Will be set by command manager
    ...
    def deserialize(cls, data: dict) -> 'GameCommand':
        ...
        command = cls(data["command_type"], data["player_index"], data["timestamp"])
...
class PlaceCardCommand(GameCommand):
    ...
    def __init__(self, player_index: int, x: int, y: int, card_rotation: int, timestamp: Optional[float] = None):
    ...
    def deserialize(cls, data: dict) -> 'PlaceCardCommand':
        command = cls(data["player_index"], data["x"], data["y"], data["card_rotation"], data["timestamp"])
        command.command_id = data["command_id"]
        command.sequence_number = data["sequence_number"]
        return command
...
class PlaceFigureCommand(GameCommand):
    ...
    def __init__(self, player_index: int, x: int, y: int, position: str, timestamp: Optional[float] = None):
    ...
    def deserialize(cls, data: dict) -> 'PlaceFigureCommand':
        command = cls(data["player_index"], data["x"], data["y"], data["position"], data["timestamp"])
        command.command_id = data["command_id"]
        command.sequence_number = data["sequence_number"]
        return command
...
class SkipActionCommand(GameCommand):
    ...
    def __init__(self, player_index: int, action_type: str, timestamp: Optional[float] = None):
    ...
    def deserialize(cls, data: dict) -> 'SkipActionCommand':
        command = cls(data["player_index"], data["action_type"], data["timestamp"])
        command.command_id = data["command_id"]
        command.sequence_number = data["sequence_number"]
        return command
...
class CommandManager:
    ...
    def add_command(self, command: GameCommand) -> None:
        ...
        logger.debug(f"Added command {command.command_type} with sequence {command.sequence_number}")
    ...
    def get_commands_since(self, sequence_number: int) -> List[GameCommand]:
        ...
        return [cmd for cmd in self.commands if cmd.sequence_number > sequence_number]
    ...
    def mark_command_pending_ack(self, command_id: str) -> None:
        ...
        self.pending_acks[command_id] = time.time()
    ...
    def get_expired_commands(self) -> List[str]:
        ...
        expired = []
        for command_id, timestamp in self.pending_acks.items():
    ...
    def clear_expired_commands(self) -> None:
        ...
        for command_id in expired:
            del self.pending_acks[command_id]
            logger.warning(f"Command {command_id} expired without acknowledgment")
...
def encode_command_message(command: GameCommand, message_type: str = "command") -> str:
    """Encode a command as a network message."""
    return encodeMessage(message_type, command.serialize())

def decode_command_message(raw_message: str) -> Optional[GameCommand]:
    """Decode a command from a network message."""
    data = decodeMessage(raw_message)
    if not data:
        return None
