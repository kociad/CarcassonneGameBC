...
class GameCommand:
    ...
    def __init__(self, commandType: str, playerIndex: int, timestamp: Optional[float] = None):
...
class PlaceCardCommand(GameCommand):
    ...
    def __init__(self, playerIndex: int, x: int, y: int, cardRotation: int, timestamp: Optional[float] = None):
...
class PlaceFigureCommand(GameCommand):
    ...
    def __init__(self, playerIndex: int, x: int, y: int, position: str, timestamp: Optional[float] = None):
...
class SkipActionCommand(GameCommand):
    ...
    def __init__(self, playerIndex: int, actionType: str, timestamp: Optional[float] = None):
...
class CommandManager:
    ...
    def addCommand(self, command: GameCommand) -> None:
        ...
        logger.debug(f"Added command {command.commandType} with sequence {command.sequenceNumber}")
    ...
    def getCommandsSince(self, sequenceNumber: int) -> List[GameCommand]:
        ...
        return [cmd for cmd in self.commands if cmd.sequenceNumber > sequenceNumber]
    ...
    def clearExpiredCommands(self) -> None:
        ...
        for commandId in expired:
            del self.pendingAcks[commandId]
            logger.warning(f"Command {commandId} expired without acknowledgment")
...
def encodeCommandMessage(command: GameCommand, messageType: str = "command") -> str:
