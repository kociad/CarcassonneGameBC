...
class MainMenuScene(Scene):
    ...
    def __init__(self, screen: pygame.Surface,
        ...
        self.continue_button = Button((center_x, center_y - 80, 200, 60),
                                     ...
                                     self.button_font,
                                     None,
                                     disabled=(self.get_game_session() is None))
        ...
        for event in events:
            if event.type == pygame.QUIT:
                ...
                if self.continue_button._is_clicked(
                    ...
                    if hasattr(session, 'lobby_completed') and network_mode in (
                            "host", "client") and not session.lobby_completed:
                    ...
                    if self.get_game_session():
                        ...
                        self.switch_scene(GameState.PREPARE)
                elif self.settings_button._is_clicked(event.pos,
                                                   y_offset=self.scroll_offset):
                    ...
                    pygame.quit()
                    exit()
                elif self.how_to_play_button._is_clicked(event.pos,
                                                    y_offset=self.scroll_offset):
    ...
    def draw_confirm_dialog(self) -> None:
        ...
        confirm_text = self.dialog_font.render("Do you want to continue?", True,
                                             (255, 255, 255))
