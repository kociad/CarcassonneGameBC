...
class ThemeDebugScene(Scene):
    ...
    def _add_color_controls(self, name: str, value: tuple) -> None:
        ...
        fields = self._build_rgb_fields(name, color_state)
        offsets = [0, 70, 140]
        self._add_control_row(label, fields, offsets=offsets, height=40)
    ...
    def _build_rgb_fields(self, name: str,
                          color_state: list[int]) -> list[InputField]:
        fields = []
        for index in range(3):
            field = InputField(
                rect=(0, 0, 60, 36),
                font=self.label_font,
                initial_text=str(color_state[index]),
                numeric=True,
                on_text_change=lambda text, i=index: self._handle_rgb_input(
                    name, color_state, i, text),
            )
            fields.append(field)
        return fields

    def _handle_rgb_input(self, name: str, color_state: list[int], index: int,
                          text: str) -> None:
        if text == "" or text == "-":
            return
        try:
        ...
        color_state[index] = value
        control = self._optional_color_controls.get(name)
        if control and not control["checkbox"].is_checked():
            return
    ...
    def _layout_preview(self) -> None:
        ...
        self.preview_left = max(
            40, (self.screen.get_width() - total_width) // 2)

        self.preview_button.rect.topleft = (self.preview_left, preview_top)
        self.preview_checkbox.rect.topleft = (self.preview_left,
                                              preview_top + row_gap)
        self.preview_slider.rect.topleft = (
            self.preview_left + column_width + column_gap,
            preview_top + row_gap,
        )
    ...
    def draw(self) -> None:
        ...
        preview_rect = preview_label.get_rect(
            center=(self.screen.get_width() // 2, 100 + offset_y))
        ...
        caption_rect = preview_caption.get_rect(
            center=(self.screen.get_width() // 2, 140 + offset_y))
        self.screen.blit(preview_caption, caption_rect)
        ...
        theme_label_rect = theme_label.get_rect(
            center=(self.screen.get_width() // 2,
                    self.preview_bottom + 40 + offset_y))
        self.screen.blit(theme_label, theme_label_rect)

        label_right = self.screen.get_width() // 2 - 20
        for row in self._control_rows:
